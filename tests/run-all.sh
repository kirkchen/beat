...
# Layer 4: Pipeline Integration (opt-in, ~50 min)
if [[ "${1:-}" == "--integration" ]]; then
    echo "=== Layer 4a: Pipeline Integration Test (new feature) ==="
    ...
    echo "=== Layer 4b: Pipeline Integration Test (modify feature) ==="
    if bash "$SCRIPT_DIR/pipeline/modify-feature/run-test.sh"; then
        PASS=$((PASS + 1))
    else
        FAIL=$((FAIL + 1))
    fi
